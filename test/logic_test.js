...
/* --- 9. 会排除宝藏的猜测被拒:范围 25~85,猜 60 → 25~60 不含78/79 → 拒 --- */
var bad6 = G.guess(g, p3, 60);
assert(!bad6.ok, '猜60(会排除元宝78/地雷79)应被拒');
eq(g.low, 25, '被拒后下界仍25');
eq(g.high, 85, '被拒后上界仍85');

/* --- 9b. 猜 50 → 50~85(仍含宝藏) --- */
var g3 = G.guess(g, p3, 50);
assert(g3.ok, '猜50应成功');
eq(g.low, 50, '猜50后下界=50');
eq(g.high, 85, '猜50后上界=85');

/* --- 10. 猜中元宝 78 结束 --- */
// 现在范围 50~85,轮到 p1。78 在范围内。让 p1 猜 78。
var g4 = G.guess(g, p1, 78);
assert(g4.ok && g4.hit === 'treasure', '猜78应命中元宝');
eq(g.status, 'finished', '游戏结束');
eq(g.winnerId, p1, '赢家是小明');
